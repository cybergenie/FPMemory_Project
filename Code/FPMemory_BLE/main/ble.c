...
static void spp_task_init(void)
{
	printf("mV\n");
    while(1){
    	if(enable_data_ntf==true){
    	uint8_t notify_data[16];
    	adc_voltage(voltage);

    	for(int i = 0; i < 8; i++)
    	{
    		notify_data[2*i] = (voltage[i]>>8)& 0x000000ff;
    		notify_data[2*i+1] = voltage[i] & 0x000000ff;
    		printf("%d mV  ",voltage[i]);
    	}
    	printf("\n");
    	//the size of notify_data[] need less than MTU size
    	esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, heart_rate_handle_table[IDX_CHAR_VAL_A],sizeof(notify_data), notify_data, false);
    	}
    	else
    	{
    		printf("no mV\n");
            ...
            if (!param->write.is_prep){
                ...
                if (heart_rate_handle_table[IDX_CHAR_CFG_A] == param->write.handle && param->write.len == 2){
                    ...
                    if (descr_value == 0x0001){
                    	// when notify enable, set the paramater
                    	ESP_LOGI(GATTS_TABLE_TAG, "notify enable");
                    	spp_gatts_if=gatts_if;
                    	spp_conn_id=param->write.conn_id;
...
void app_main()
{
    ...
    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(spp_mtu_size);
    if (local_mtu_ret){
        ESP_LOGE(GATTS_TABLE_TAG, "set local  MTU failed, error code = %x", local_mtu_ret);
    }

    //add the user task "spp_task_init()" to main.
    spp_task_init();
    return;
}
